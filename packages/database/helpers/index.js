...
exports.mutationSet = (knex, tb, options = {
  creUsrId: true,
  updUsrId: true,
  creDt: true,
  updDt: true
}) => {
  const { creUsrId, updUsrId, creDt, updDt } = { ...DEFAULT_MUTATION_SET_OPTS, ...options };
  if (creUsrId) tb.integer('creUsrId').comment('Create User Id');
  if (updUsrId) tb.integer('updUsrId').comment('Create User Id');
  if (creDt) tb.timestamp('creDt').defaultTo(knex.fn.now()).comment('Create Date');
  if (updDt) tb.timestamp('updDt').defaultTo(knex.fn.now()).comment('Update Date');
  return tb;
}
